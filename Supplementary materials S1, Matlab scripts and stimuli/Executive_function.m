...
interTrialInterval = 3;  % Time between two trials, in seconds`
Screen('Preference', 'SkipSyncTests', 1);                 % Skip sync tests

%%% Gather info about available displays
screens = Screen('Screens');   % Get the scr een numbers
...
prompt = {'Output file', 'Subject ID', 'Gender', 'Session','width','height','maxtime','maxtrial'};
defaults = {'RTLeftRight', '', 'M', '1','400','700','15','36'};

answer = inputdlg(prompt, 'RTLeftRight', 2, defaults);  % Get experimenter's answers
[s_output, s_subjID, s_gender, s_Session,w,h,l,t] = deal(answer{:});
% All answers are strings, hence the 's_'
width=str2num(w);
height=str2num(h);
respLimit = str2num(l); 
maxTrials = str2num(t);
outputname = [s_output, s_gender, s_subjID, s_Session, w, h, l, t '.csv'];  % Generate output file name
...
rect = [0, 0, width, height];     % Make 100x100 rectangle (at the origin 0,0)taille
 ...
 %Look if currTrial is even or odd with modulo 2
  ...
  while correct==0
    ...
    while ~any(touch)
      ...
      % ...if so or if monkey abandoned (time > limit), cross
      if keyIsDown
        sca;
        fclose(outfile);
        return;
    ...
    if rectPos(1)-80 < xClick && xClick < rectPos(3)+80 && rectPos(2)-80 < yClick && yClick < rectPos(4)+80 
